...
numGreaterThanTen = length . filter (\ (s, i) -> i>10)
...
sumAndLen = foldr (\ e (e2, i2) -> (e2 + e, i2 + 1)) (0, 0)
...
mapTree f = foldTree Leaf (\ e1 e2 e3 -> (Node e1 (f e2) e3))
...
flatten :: Tree a -> [a]
flatten Leaf = []
flatten (Node e1 x e2) = flatten e1 ++ [x] ++ flatten e2
...
pair cmp1 cmp2 (a1, b1) (a2, b2) = case cmp1 a1 a2 of
                                        LT -> LT
                                        GT -> GT
                                        EQ -> case cmp2 b1 b2 of
                                            LT -> LT
                                            GT -> GT
                                            _  -> EQ 
...
sortOnReverseSndLength l = mergeSortWith (invert compare `on` (length . snd)) l 
...
colourIn p1 c1 c2 (x,y) = if p1 (x,y) then c1 else c2
...
mapPicture2 f p1 p2 point = f (p1 point) (p2 point)
...
pictureApply f1 f2 (x,y) =  (f1 (x,y)) (f2 (x,y))   
...
mapPicture2' :: (a -> b -> c) -> Picture a -> Picture b -> Picture c
mapPicture2' f p1 p2 p = pictureApply (everywhere (f $ p1 p)) p2 p
...
flipTopBottom p1 = transform (\ (x,y) -> if y > 0 then (x,-y) else (x,y)) p1 


flipLeftRight :: Picture a -> Picture a
flipLeftRight p1 = transform (\ (x,y) -> if x > 0 then (-x,y) else (x,y)) p1 
...
sequ (End b) f = f b
sequ (Output b p) f = Output b (sequ p f) 
sequ (Input f1) f2 =  Input (\ a ->  sequ (f1 a) f2) 
...
echoFromSequ = sequ input (\e -> output e)
...
chatBot = Output ("Hello! What is your name?") (Input (\ e -> Output ("Hello " ++ e ++ "!") (End ())))
...
eqProcess (End ())       (End ()) = True
  -- are these processes equal?
eqProcess (Output b1 k1) (Output b2 k2) = b1 == b2 && eqProcess k1 k2
...
eqProcess (Input k1)     (Input k2) = eqProcess (k1 True) (k2 True) && eqProcess (k1 False) (k2 False) 
  -- what are the possible input values of t ype Bool?
eqProcess _ _ =
  False
